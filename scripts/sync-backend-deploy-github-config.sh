#!/usr/bin/env bash
# Sync gamyaboutique GitHub Actions variables/secrets from Terraform outputs.
# Requires: gh CLI, jq, terraform (already applied in environments/dev).
...
if ! command -v gh >/dev/null 2>&1; then
  echo "gh CLI not found; install https://cli.github.com/ or set GitHub vars manually." >&2
  exit 1
fi
...
echo "Done. EC2_INSTANCE_ID=${EC2_INSTANCE_ID} EC2_HOST=${EC2_HOST} DEPLOY_BUCKET=${DEPLOY_BUCKET}"
